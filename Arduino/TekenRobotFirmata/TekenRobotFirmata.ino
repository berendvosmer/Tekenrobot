...
#define PENUP             5
#define PENDOWN           6
#define PIVOT_ON_PEN      7
#define PIVOT_ON_CENTER   8
#define RELEASE_STEPPERS  9
...
void penDown() {
  analogWriteCallback(SERVO_PIN, pen_down);
  delay(500);
}
...
void reset() {
  setPinModeCallback(2, PIN_MODE_IGNORE);
  setPinModeCallback(3, PIN_MODE_IGNORE);
  setPinModeCallback(4, PIN_MODE_IGNORE);
  setPinModeCallback(5, PIN_MODE_IGNORE);
  setPinModeCallback(6, PIN_MODE_IGNORE);
  setPinModeCallback(7, PIN_MODE_IGNORE);
  setPinModeCallback(8, PIN_MODE_IGNORE);
  setPinModeCallback(9, PIN_MODE_IGNORE);
  setPinModeCallback(10, PIN_MODE_IGNORE);
  setPinModeCallback(11, PIN_MODE_IGNORE);
  setPinModeCallback(12, PIN_MODE_IGNORE);
  setPinModeCallback(13, OUTPUT);

  setPinModeCallback(SERVO_PIN, PIN_MODE_SERVO);
 ...
 * Helpers
 *============================================================================*/
uint8_t decode8BitUnsignedInteger(byte arg1, byte arg2)
{
  uint8_t result = (uint8_t)arg1 | (uint8_t)arg2 << 7;

  return result;
}

void encode8BitUnsignedInteger(uint8_t value, byte pdata[])
{
  pdata[0] = value & 0x7f;
  pdata[1] = (value >> 7) & 0x7f;
}

uint16_t decode16BitUnsignedInteger(byte arg1, byte arg2, byte arg3)
{
  uint16_t result = (uint16_t)arg1 | (uint16_t)arg2 << 7 | (uint16_t)arg3 << 14;

  return result;
}

void encode16BitUnsignedInteger(uint16_t value, byte pdata[])
{
  pdata[0] = value & 0x7f;
  pdata[1] = (value >> 7) & 0x7f;
    ...
    case ROBOT_CMD:
      ...
      switch (mode) {
        case FORWARD:
          ...
          distance = decode16BitUnsignedInteger(argv[1], argv[2], argv[3]);
          forward(distance);
          break;
        case BACKWARD:
          distance = decode16BitUnsignedInteger(argv[1], argv[2], argv[3]);
          ...
          break;
        case LEFT:
          angle = decode8BitUnsignedInteger(argv[1], argv[2]);
          left(angle);
          break;
        case RIGHT:
          angle = decode8BitUnsignedInteger(argv[1], argv[2]);
        ...
        case PIVOT_ON_PEN:
          setPivotOnPen();
          break;
        case PIVOT_ON_CENTER:
          setPivotOnCenter();
          break;
        case RELEASE_STEPPERS:
          release();
          break;
        default:
          return;
